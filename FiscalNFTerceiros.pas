...
type
  TfFiscalNFTerceiros = class(TuniFrame)
    ...
    NotasValor_BCICMS: TBCDField;
    NotasValor_ICMS: TBCDField;
    NotasAliquota_ICMSSub: TBCDField;
    NotasValor_BCICMSST: TBCDField;
    NotasValor_ICMSST: TBCDField;
    NotasValor_Produtos: TBCDField;
    NotasValor_Frete: TBCDField;
    NotasValor_Seguro: TBCDField;
    NotasValor_Despesas: TBCDField;
    NotasValor_BCIPI: TBCDField;
    NotasValor_IPI: TBCDField;
    NotasValor_TotalNota: TBCDField;
    NotasValor_RateioDespesas: TBCDField;
    NotasValor_MVA: TBCDField;
    NotasValor_ICMSReducao: TBCDField;
    NotasValor_II: TBCDField;
    NotasValor_DUMPING: TBCDField;
    NotasValor_Descontos: TBCDField;
    ...
    NotasValor_BCICMSApuracao: TBCDField;
    NotasValor_ICMSApuracao: TBCDField;
    NotasValor_MediaBCR: TBCDField;
    ...
    NotasValor_Comissao: TBCDField;
    NotasTotal_Impostos: TBCDField;
    NotasValor_BCICMSDest: TBCDField;
    NotasValor_ICMSDest: TBCDField;
    NotasValor_DIFAL: TBCDField;
    NotasValor_DIFALOrig: TBCDField;
    NotasValor_DIFALDest: TBCDField;
    NotasValor_FCPDest: TBCDField;
    NotasValor_FCPICMSDest: TBCDField;
    NotasValor_FCPICMSOrig: TBCDField;
    ...
    NotasValor_BCDIFALST: TBCDField;
    ...
    NotasValor_BCIBS: TBCDField;
    NotasValor_IBS: TBCDField;
    NotasValor_BCCBS: TBCDField;
    NotasValor_CBS: TBCDField;
    NotasValor_BCIS: TBCDField;
    NotasValor_IS: TBCDField;
    NotasValor_IsentasICMSST: TBCDField;
    NotasValor_OutrasICMSST: TBCDField;
    NotasValor_ProdutosOrig: TBCDField;
    ...
    NotasOperacao_Descricao: TStringField;
...
begin
      ...
      with Itens do begin
           sql.Clear;
           sql.Add('select *');
           sql.add('      ,Estoque_Minimo = isnull((select Estoque_MinimoPerc from Produtos where Codigo = NotasItens.Codigo_Mercadoria), 0)');
...
begin
     try
         ...
         FrameItem         := TfFiscalNFTerceirosItens.create(uniTabSheet1, NotasNota.asinteger, 0, 'Adicionar');
                               ...
                               try
                                  with ttmp do begin
                                       ...
                                       while not Itens.eof do begin
                                             if ItensEstoque_Minimo.asfloat > 0 then begin
                                                mEstMin := Percentual(EstoqueProduto(ItensCodigo_Mercadoria.AsInteger)-ItensQuantidade.AsFloat, ItensEstoque_Minimo.AsFloat);
                                                sql.add('update Produtos set Estoque_Minimo = '+floattostr(mEstMin)+' where Codigo = '+ItensCodigo_Mercadoria.asstring );
                                             end;
                                             Itens.next;
                                       end;
...
begin
     ...
     try
        ...
        with ttmp do begin
             ...
             ParamByName('pProcesso').value := NotasProcesso.Value;
        ...
        TfDialogo.Execute(UniApplication, 'Erro', E.Message);
        //MessageDlgN('Falha desconhecida, não pode salvar a nota fiscal!'+#13+E.Message, mtError, [mbOK]);
     end;
end;
...
begin
     // Verifica se o data da nota esta em um período bloqueado.
     if PeriodoBloqueado then begin
        TfDialogo.Execute(UniApplication, 'Bloqueado', 'Esta nota fiscal não pode ser alterada, Esta dentro de um período fechado');
        Abort;
     end;
     // Verifica se o data da nota esta teve laçamento financeiro baixado.
     if Baixado then begin
        TfDialogo.Execute(UniApplication, 'Baixado', 'Esta nota fiscal não pode ser alterada, Lançamento financeiro baixado :'+ttmp.fieldbyname('Titulo').asstring+ ' de '+ttmp.fieldbyname('Data').asstring+', estorne a baixa primeiro');
        Abort;
     end;
     // Verifica se o algum item da nota de entrada teve movimentação posterior.
     if Movimentado then begin
