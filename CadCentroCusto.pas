...
type
  TfCadCentroCusto = class(TUniFrame)
...
procedure TfCadCentroCusto.bAdicionarClick(Sender: TObject);
begin
...
procedure TfCadCentroCusto.bCancelarClick(Sender: TObject);
begin
     CentroCusto.Cancel;
     LigaBotoes(true);
end;

procedure TfCadCentroCusto.bEditarClick(Sender: TObject);
begin
     try
         LigaBotoes(false);
         CentroCusto.Edit;
...
procedure TfCadCentroCusto.bExcluirClick(Sender: TObject);
begin
     with CentroCusto do begin
...
procedure TfCadCentroCusto.bFecharClick(Sender: TObject);
begin
      MainForm.PagePrincipal.Pages[MainForm.PagePrincipal.ActivePageIndex].Close;
end;

procedure TfCadCentroCusto.bSalvarClick(Sender: TObject);
begin
      with CentroCusto do begin
           if (State = dsInsert) then begin
              CentroCusto.FieldByName('Codigo').Value := GeraCodigo('CentroCusto', 'Codigo');
           end;
           if (State = dsInsert) and (Existe(CentroCusto, 'Codigo', FieldByName('Codigo').AsString)) then begin
              MessageDlg('Já existe um "Centro de Custos" cadastrado com esse codigo!'+#13+#13, mtError, [mbOK]);
              cCodigo.SetFocus;
              Abort;
           end;
...
procedure TfCadCentroCusto.CentroCustoBeforePost(DataSet: TDataSet);
begin
      LogDados(DataSet, DataSet.FieldByName('Nome').AsString, EstadoTabela(DataSet));
end;

procedure TfCadCentroCusto.CentroCustoBeforeDelete(DataSet: TDataSet);
begin
       LogDados(DataSet, DataSet.FieldByName('Nome').AsString,'Delete');
end;

procedure TfCadCentroCusto.LigaBotoes(Estado:boolean);
begin
...
procedure TfCadCentroCusto.UniFrameCreate(Sender: TObject);
var
  i:integer;
begin
...
procedure TfCadCentroCusto.UniFrameDestroy(Sender: TObject);
begin
      CentroCusto.Close;
end;

procedure TfCadCentroCusto.cPesquisaKeyDown(Sender: TObject; var Key: Word;Shift: TShiftState);
begin
